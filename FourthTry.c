...
#include<math.h>	    // necessary for all the conventional arithmetical functions.
#include<time.h>	    // necessary to provide current LocalTime (as is followed by LINUX) of the system.

#define NO_OF_T 16      /* NO_OF_T here is a MACRO(essentially a constant) used to represent the total number         
                         of threads used in this multi-threading approach.*/

#define MIN_PID 100     /* MIN_PID here is a MACRO(essentially a constant) used to specify the minimum value of     
                        process id.*/

#define MAX_PID 1000    /* MAX_PID here is a MACRO(essentially a constant) used to specify the maximum value of
                        process id.*/
...
void* threadFunction(void* arg) {       // defines the functionality performed by threadFunction.
        ...
        int tid = *((int *)arg);        /*type casting the void pointer argument to integer pointer so as to get the 
	...
	int id = allocate_pid();        // assigning a process id to the thread.
        ...
        if(id == -1) printf("No PID is available. Please try again later.\n");  // fails to get a valid process id.
                ...
                printf("Thread %3d  has been allocated the following process id:\t%3d\n", tid, id + MIN_PID);
...
int main(void) {
        ...
        srand(time(NULL));
        for(i = 0;i < NO_OF_T;i++) {
                if(pthread_create(&thread_arr[i], NULL, threadFunction, (void*)&i) < 0) {       /* Creating a new
                                                                 thread pointed through 'thread_arr[i]' element.*/
                
        /*Indicates an error in thread creation. [Since pthread_create will return a non-zero value which is often 
        less than zero in most cases.]*/
       ...
       for(i=0;i < NO_OF_T ;i++) {
        ...
        printf("PROGRAM EXECUTION SUCCESSFUL\n");          /* Indicates a successful termination of thread via 
                                                           the parent process.*/
